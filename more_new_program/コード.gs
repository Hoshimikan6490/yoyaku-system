 ...
 // 手動初期設定完了 /////////////////////////////////////////////////////////////////////////////
  ...
  // 「Mainprogram」が実行されたときに動く内容を設定
   ...
   // 確認メール送信
     ...
     // 取得したHTMLに変数を代入
       let html = HtmlService.createHtmlOutputFromFile("yoyaku_success").getContent();
   ...
   // 予約日に【予約不可】があるときに
     ...
     // 確認メール送信
       // HTMLを取得
         doGet_yoyaku_canot();
       // 取得したHTMLに変数を代入
         let html = HtmlService.createHtmlOutputFromFile("yoyaku_canot").getContent();
       // 取得したHTMLの一番最初の「Name」を変数の「Name」の中身に置き換え（以降同様）
         let html2 = html.replace("Name", Name);
       ...
       if (Calendar.getEventsForDay(new Date(Yoyaku_day)).length!== Limit) {
         for (Count=1;Count<Limit+1;Count++) {
           ...
           if (!Calendar.getEventsForDay(new Date(Yoyaku_day), {search: Count_character }).length) {
             // Limit数を予約順に変換
         ...
         // HTMLを取得
           doGet_yoyaku_full();
         // 取得したHTMLに変数を代入
           let html = HtmlService.createHtmlOutputFromFile("yoyaku_full").getContent();
         // 取得したHTMLの一番最初の「Name」を変数の「Name」の中身に置き換え（以降同様）
           let html2 = html.replace("Name", Name);
           let html3 = html2.replace("Calendar_URL", Calendar_URL);
           let html_perfect = html3.replace("Forms_URL", Forms_URL);
...
　　　let Cancel_program = function (Cancel_NO) {
   // 指定された予約番号に当てはまる統計データシートの予約番号を変数「database_Cancel_NO」に代入
     let Database_Cancel_NO = Yoyaku_list_sheet.getRange(Cancel_NO,1).getValue()
   // 指定された予約番号に当てはまる統計データシートのメールアドレスを変数「Database_cancel_Email」に代入
     let Database_cancel_Email = Yoyaku_list_sheet.getRange(Cancel_NO,4).getValue()
   // 指定された予約番号に当てはまる統計データシートの予約順を変数「Database_cancel_Ranking」に代入
     let Database_cancel_Ranking = Yoyaku_list_sheet.getRange(Cancel_NO,5).getValue()
   //ログを書く
   ...
   // OKパターン
     let Can_Cleaning = function(Name) {
       ...
       let Find_events = 　Calendar.getEventsForDay(new Date(Cancel_day), {search: Database_cancel_Ranking_character})
       ...
       // ログを書く
         ...
         //　自動返信メール
           ...
           // 取得したHTMLに変数を代入
             let html = HtmlService.createHtmlOutputFromFile("cancel_success").getContent();
           // 取得したHTMLの一番最初の「Name」を変数の「Name」の中身に置き換え（以降同様）
             let html2 = html.replace("Name", Name);
   ...
   // 失敗時のメール送信
     ...
     //　自動返信メール
       // HTMLを取得
         doGet_cancel_canot();
       // 取得したHTMLに変数を代入
         let html = HtmlService.createHtmlOutputFromFile("cancel_canot").getContent();
         // 取得したHTMLの一番最初の「Name」を変数の「Name」の中身に置き換え（以降同様）
         let html2 = html.replace("Name", Name);
         let html3 = html2.replace("Cancel_day", Cancel_day);
     ...
     if (Number(Cancel_NO) === Number(Database_Cancel_NO)) {
       // キャンセル時のメールアドレスと登録時のメールアドレスが同じなら、
         if (Email === Database_cancel_Email) {
           let Cancel_finished = Yoyaku_list_sheet.getRange(Cancel_NO, 6).getValue();
